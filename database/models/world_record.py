...
class WorldRecord:
    ...
    def get_all(approved_only=True):
        query = """
            SELECT wr.*, a.firstname, a.lastname, n.name as npc_name, r.name as region_name,
                   u.username as approved_by_username
            FROM world_records wr
            LEFT JOIN athletes a ON wr.sdms = a.sdms
            LEFT JOIN npcs n ON wr.npc = n.code
            LEFT JOIN regions r ON wr.region_code = r.code
    ...
    @staticmethod
    def check_existing_record(event, class_name, record_type, region_code=None):
        if record_type == 'WR':
            return execute_one("""
            ...
            return execute_one("""
                SELECT * FROM world_records 
                WHERE event = %s AND athlete_class = %s AND record_type = 'AR' 
                AND region_code = %s AND approved = TRUE
                ORDER BY performance DESC LIMIT 1
            """, (event, class_name, region_code))
        return None
